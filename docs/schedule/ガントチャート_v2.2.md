# ガントチャート v2.2（現状実装反映版）

**プロジェクト：** 田島組 卒業制作（男の身だしなみアプリ）
**基準日：** 2026-09-01
**作成根拠：** `develop`（`5cb60d7`）・`app/` の現行実装・`scripts/` のローカル検証・GitHub Issues（自動同期）
**主目標：** **4画面をローカルデータで通し操作できる最小MVPを Cloudflare Pages で公開する。主要機能の実装は `develop` に反映済みで、公開URL・実機・薬機法確認を残りのゲートとして管理する。**
...
## 1. 実装スナップショット（2026-09-01）

### 1.1 現行の利用フロー

現行の `develop`（`5cb60d7`）では、M-MVPの4画面に導入画面を加えた次の流れが実装されています。
...
| PWA・公開設定 | 構成済み | `app/manifest.webmanifest`、`app/sw.js`、Cloudflare Pages用workflow。Service Workerキャッシュは `v32` |
| ローカル品質確認 | 合格 | `app/js`・`app/data`配下のJS構文チェック、11本・83件のNodeテスト、3本のデータ検証を実行し全件成功 |

### 1.3 現在の判断

- **M-MVPの主要機能は `develop` に統合済み**です。以前の「5問UIと診断ロジックが未結線」という記述は現状には当てはまりません。
- `App.S1_TOTAL` と画面は5問のMVPセットで結線済みです。診断ロジック内に残る23問分の `pointTable` への拡張は、MVP後の条件付きタスク（#59）として扱います。
- GitHub Issueの「完了」は、公開URL・実機・人間による通しデモまでを保証しません。Issue同期表と、下記の実装／外部確認ゲートを分けて判定します。

---

## 2. 外部確認ゲートと残課題

| タスク | 現状（2026-09-01） | 次に行うこと |
| --- | --- | --- |
...
| #46 M-MVP公開 | Issue同期上は完了。Cloudflare Pagesの自動デプロイworkflowは構成済みだが、この更新では本番URLへの接続を確認していない | `main` へのリリース後、公開URLで4画面の通し操作を確認する |
| #90 匿名利用統計 | 現行 `app/` に収集・送信処理は見当たらず、Issueは未完了 | 同意・プライバシー影響を決めてから必要最小限を実装する |
| #47 商品マスター拡充 | 現在15件。30〜50件への拡充は未着手 | MVPの安定を確認後にデータを追加する |
...
```mermaid
gantt
    title 田島組 卒業制作（2026-09-01現状実装 / M-MVP公開ゲート）
    dateFormat  YYYY-MM-DD
    axisFormat  %m/%d
    ...
    機能テスト・静的a11y・データ検証             :done, localcheck, 2026-09-01, 2026-09-01
    ...
    develop通しデモの実施記録（#41）              :active, demo, 2026-09-01, 2026-09-01
    薬機法表現チェック（#43）                     :crit, compliance, 2026-09-01, 2026-09-01
    iOS／Android実機PWA確認（#108）               :crit, device, 2026-09-01, 2026-09-01
    Cloudflare Pages公開URLの通し確認（#46）      :crit, releasecheck, 2026-09-01, 2026-09-01
    匿名利用統計の方針・実装（#90）               :metrics, 2026-09-01, 2026-09-01
...
> 公開確認ゲートの日付は、未実施の外部確認を識別するため基準日を置いています。実施日が確定したら、実績日に更新してください。#59の23問拡張はMVPのクリティカルパスから外しています。
...
| --- | --- | --- | --- |
| **完了** | SC-00〜S4の画面フロー | `develop` に実装済み。5問診断→ロードマップ→商品→記録を結線 | `develop` の実ブラウザ通しデモ結果を記録する |
...
| **確認待ち** | #42 機能／アクセシビリティ | ローカル自動テスト83件と静的点検は合格 | 実ブラウザで操作性・a11yを確認 |
| **未完了** | #43 薬機法表現 | Issue未完了 | 全画面と商品データを人間レビュー |
| **未完了** | #108 実機PWA | Issue未完了 | iOS Safari／Android Chromeの公開URLで確認 |
| **確認待ち** | #46 Cloudflare Pages公開 | Issue同期上は完了、workflowは構成済み | `main` リリース後の公開URLを確認 |
...
6. JS構文チェック、機能テスト83件、データ検証3件がすべて成功。
...
4. `main` リリース後のCloudflare Pages公開URLで、4画面の通し操作を確認（#46）。
...
| リスク | 影響 | 対策・判断基準 |
| --- | --- | --- |
| 外部確認が未記録 | 実装済みでも公開可否を判断できない | #41/#43/#108/#46を実施し、URL・端末・結果を記録する |
| 発表準備の後ろ倒し | 資料とデモの確認が本番直前に集中する | #44を10/10、#45を10/24までに終え、10/27〜11/07を改善・予備期間として確保する |
| Cloudflare Pages設定不足 | workflowが動いても本番公開できない | `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`、`CLOUDFLARE_PAGES_PROJECT_NAME`、production branch `main` を確認する |
